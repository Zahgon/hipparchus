...
public class SQPOptimizerS extends AbstractSQPOptimizer {
    ...
    private RealVector updateRj(RealMatrix H, RealVector y, RealVector dx, RealVector dy, RealVector r, double additional) {
        //r = updateRj(currentH,dx,y,u,r,sigm);
    ...
    private LagrangeSolution solveQP(RealVector x, RealVector y, double rho) {
        ...
        if (getIqConstraint() != null) {
            mi = getIqConstraint().dimY();
            violated = inequalityEval.subtract(getIqConstraint().getLowerBound()).getMinValue() <= getSettings().getEps() || y.getMaxValue() >= 0;
        }
        // violated = true;
        if (me > 0 || violated) {
            add = 1;
    ...
    private RealMatrix BFGSFormula(RealMatrix oldH, RealVector dx, double alfa, RealVector newGrad, RealVector oldGrad) {
        ...
        EigenDecompositionSymmetric dsX = new EigenDecompositionSymmetric(Hnew, getMatrixDecompositionTolerance().getEpsMatrixDecomposition(), true);
