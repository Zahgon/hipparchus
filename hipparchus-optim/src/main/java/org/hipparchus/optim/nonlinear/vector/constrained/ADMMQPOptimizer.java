...
public class ADMMQPOptimizer extends QPOptimizer {
    ...
    private ADMMQPSolution polish(RealMatrix H, RealMatrix A, RealVector q, RealVector lb, RealVector ub, RealVector x, RealVector y, RealVector z) {
        ...
        for (int j = 0; j < A.getRowDimension(); j++) {
            if (z.getEntry(j) - lb.getEntry(j) < -y.getEntry(j)) {
                // lower-active
        ...
        for (int j = 0; j < A.getRowDimension(); j++) {
            if (-z.getEntry(j) + ub.getEntry(j) < y.getEntry(j)) {
                // lower-active
                Aentry.add(A.getRow(j));
        ...
        if (!Aentry.isEmpty()) {
            ...
            solver.initialize(H, Aactive, q, 0, lub, lub, settings.getSigma(), settings.getSigma(), settings.getAlpha());
    ...
    private boolean manageRho(int me, double rp, double rd, double maxPrimal, double maxDual) {
        ...
        if (settings.updateRho()) {
            ...
            double rhonew = FastMath.min(FastMath.max(rho * FastMath.sqrt((rp * maxDual) / (rd * maxPrimal)), settings.getRhoMin()), settings.getRhoMax());
