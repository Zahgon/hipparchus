...
public abstract class FieldJacobiElliptic<T extends CalculusFieldElement<T>> {
    ...
    private T arcps(final T x, final T deltaQP, final T deltaRP) {
        ...
        return FastMath.copySign(1.0, rf.getReal()) * FastMath.copySign(1.0, x.getReal()) < 0 ? rf.negate() : rf;
    }
    ...
    private T arcsp(final T x, final T deltaQP, final T deltaRP) {
        ...
        final T x2 = x.square();
        return x.multiply(CarlsonEllipticIntegral.rF(x.getField().getOne(), deltaQP.multiply(x2).add(1), deltaRP.multiply(x2).add(1)));
    }
    ...
    private T arcpqNoDivision(final T x, final T deltaQP, final T deltaRQ) {
        ...
        return FastMath.copySign(1.0, x.getReal()) < 0 ? LegendreEllipticIntegral.bigK(getM()).twice().subtract(positive) : positive;
    }
}
